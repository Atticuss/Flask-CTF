    ...
    try:
        with establish_admin_conn() as admin_conn:
            with admin_conn.cursor() as curs:
                curs.execute('drop user %s' % user)
                admin_conn.commit()
                curs.execute('drop login %s' % user)
                admin_conn.commit()
    except:
        pass
...
def build_schema():
    ...
    with establish_admin_conn() as admin_conn:
        with admin_conn.cursor() as curs:
            ...
            curs.execute('create table students (ID int PRIMARY KEY NOT NULL IDENTITY(1, 1), email varchar(1000))')
            admin_conn.commit()
